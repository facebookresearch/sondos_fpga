...
module aurora_firefly_0_wrapper
(
...
aurora_firefly_0 inst_aurora (
   ...
   .txp                        (ff0_tx_p           ),
   .txn                        (ff0_tx_n           ),
   .gt_refclk1_p               (ff0_refclk_p       ),
   .gt_refclk1_n               (ff0_refclk_n       ),
   .gt_refclk1_out             (                   ),
   .hard_err                   (hard_err           ),
   .soft_err                   (soft_err           ),
...
`ifdef SONDOS_SIM
        clk_counter <= (user_reset)? 27'd256:
                       (reset_state!=ST_USER_RESET && reset_next_state == ST_USER_RESET)? '1:
                       (reset_state!=ST_PMA_INIT && reset_next_state == ST_PMA_INIT)? 27'd32 :
                       (|clk_counter)? clk_counter - 1'b1: 0;
`else
        clk_counter <= (user_reset)? 27'd256:

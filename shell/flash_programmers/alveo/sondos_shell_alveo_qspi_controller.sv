...
module sondos_shell_alveo_qspi_controller(
   input clk,
   input reset,
   input done_beat,
...
// all read/write transactions with SW would be on 4KB (page) boundaries, so starting_address and ending_address are expected to have bits [11:0] = 0
//
...
typedef enum logic [5:0] {
   ...
   ST_CHECK_FOR_WRITE_DONE      = 6'b001_111, // if not last write, go to status check
   ...
   ST_CHECK_FOR_ERASE_DONE      = 6'b010_111, // if not last erase, go to status check
...
assign data_to_host_valid = ((state == ST_READ_DATA) & (clk_counter[9:0] == 10'h00B)) | (state == ST_LAST_READ_DATA);

assign increment_read_address = (clk_counter[4:0] == 5'h0A);
...
assign data_from_host_taken = ((state==ST_WRITE_DESELECT_TIME) & (clk_counter == TIC_24_CYCLES)) |
                              ((state==ST_SEND_WRITE_DATA) & (clk_counter[9:0] == 10'h00C) & ~(clk_counter == TIC_576_CYCLES));


assign flash_status_ready = (clk_counter==TIC_FLASH_STATUS) & (~spi_io_0_i_r2[1]) & (state==ST_READ_STATUS_DATA);
...
always_ff @(posedge clk) begin
   ...
   current_address <= (fsm_start)? starting_address :
                     (((state==ST_READ_DATA) | (state==ST_SEND_WRITE_DATA)) & increment_read_address)? current_address + 1'b1:
                     (state==ST_CHECK_FOR_ERASE_DONE)? current_address + {1'b1,16'd0}: current_address;
   ...
   erased_all_pages <= (state==ST_IDLE)? 1'b0 : erased_all_pages | (current_address[31:16] == ending_address[31:16]);

   write_nibble_select <= (write_clk_tic)? clk_counter[4] : write_nibble_select;

   spi_io_0_o[0] <= ((state==ST_SEND_WRITE_DATA) & write_nibble_select)? write_data_buffer[0] :
                    (state==ST_SEND_WRITE_DATA)?  write_data_buffer[4] : command_buffer[39];
   spi_io_0_o[3:1] <= (write_nibble_select)? write_data_buffer[3:1] : write_data_buffer[7:5];

   half_byte_buffer <= (read_clk_tic)? spi_io_0_i_r2 : half_byte_buffer;
   read_data_buffer <= ((state==ST_READ_DATA) & increment_read_address)? {half_byte_buffer,spi_io_0_i_r2,read_data_buffer[255:8]} : read_data_buffer;

   write_data_buffer <= (data_from_host_taken)? data_from_host :
                        ((state==ST_SEND_WRITE_DATA) & write_clk_tic & write_nibble_select)? {8'd0,write_data_buffer[255:8]} : write_data_buffer;

   command_buffer <= (state==ST_PREPARE_FOR_READ_STATUS)? {READ_STATUS_COMMAND,current_address}:
                     (state==ST_PREPARE_FOR_READ_DATA)? {READ_COMMAND,current_address}:
                     (state==ST_PREPARE_FOR_WRITE)? {WRITE_ENABLE_COMMAND,current_address}:
                     (state==ST_PREPARE_FOR_RESET)? {RESET_ENABLE_COMMAND,current_address}:
                     (state==ST_PREPARE_FOR_ERASE)? {WRITE_ENABLE_COMMAND,current_address}: // enabling erase uses same command as enabling write
                     (state==ST_WRITE_DESELECT_TIME)? {WRITE_COMMAND,current_address}:
                     (state==ST_ERASE_DESELECT_TIME)? {ERASE_COMMAND,current_address}:
                     (state==ST_RESET_DESELECT_TIME)? {RESET_COMMAND,current_address}:
...
////////////////////////////////////////////////////////////////////
   ...
   always_comb begin
      case(state)
         ...
         ST_READ_DATA           :   if((current_address == ending_address) & increment_read_address) next_state = ST_LAST_READ_DATA;
